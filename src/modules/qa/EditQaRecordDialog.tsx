...
export function EditQaRecordDialog({ target, onClose, onSaved }: Props) {
  ...
  const hasLogin = target.kind === 'gestor' || target.kind === 'afiliado' || (target.kind === 'tenant' && !!target.record.userId)
  ...
  const handleSave = async () => {
    ...
    try {
      if (target.kind === 'owner') {
        ...
        if (password && target.record.userId) {
        ...
        <div className="space-y-4">
          ...
          {hasLogin && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">{t('edit.newPasswordHint')}</p>
            </div>
          )}
        </div>
        <DialogFooter>
