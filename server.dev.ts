...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.startsWith('/api/qa-update-user')) {
      const { default: handler } = await import('./api/qa-update-user.js')
      return handler(vReq as never, vRes as never)
    }
...
server.listen(PORT, () => {
  ...
  console.log('[dev-api] Proxying: /api/checkout  /api/asaas-webhook  /api/verify-asaas-subscription  /api/create-affiliate-profile  /api/qa-create-user  /api/qa-delete-user  /api/qa-update-user')
})
